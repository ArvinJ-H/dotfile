...
> **Private overlay**: A separate repo adds 3 more meta-skills (`create`, `ops`, `meta`), standalone `a11y`, project-specific skills, and additional hooks. The public repo works standalone; the overlay extends it.
...
A: `settings.json` is generated by merging public + private overlay via `jq`. Re-run `setup.sh symlinks` to re-merge, or edit `~/.claude/settings.json` directly (local changes are overwritten on next merge).
