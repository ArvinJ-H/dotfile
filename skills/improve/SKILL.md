...
2. Parse the structured output — each line is tagged:
   ...
   - `[MIRROR]` → core↔premium diverged for a mirror-policy skill. Disposition: **Sync** (run `mirror-sync.sh --sync`).
...
- **Skip** core↔premium shared skills — these are intentional (managed by `mirror-manifest.txt`).
- Substantial overlap between non-shared, non-mirrored skills → disposition: **Merge**.
...
- **Sync**: Mirror diverged. Run `mirror-sync.sh --sync`.
...
- **Sync**: Run `{project}/.claude/scripts/mirror-sync.sh --sync` to copy core→premium for mirror-policy skills.
- **Archive** (global): `mv skills/{name}/ skills/_archived/{name}/`. Create `_archived/` if needed.
- **Delete** (project): Remove skill directory. Update `mirror-manifest.txt` if the skill was listed. Note deletion in commit message.
